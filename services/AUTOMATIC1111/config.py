...
def replace_if_invalid(
    value: str, replacement: str, pattern: str | re.Pattern[str]
) -> str:
...
def check_and_replace_config(config_file: str, target_file: str = None):
    ...
    for k, def_val in DEFAULT_OUTDIRS.items():
        if k not in data:
            ...
            data[k] = replace_if_invalid(
                value=data[k], replacement=def_val, pattern=RE_VALID_OUTDIR
            )
    ...
    # Check and fix SD config
    for k, def_val in DEFAULT_SD_CONFIG.items():
        if k not in data:
            data[k] = def_val
